...
if page == "Overview":
    ...
    if submit:
        ...
        # FIXED: Create a helper function to ensure consistent naming
        def get_disaster_name(cls):
            name = disaster_mapping.get(cls)
            if name is None:
                try:
                    cls_int = int(cls)
                    name = disaster_mapping.get(cls_int)
                except (ValueError, TypeError):
                    pass
                if name is None:
                    name = f"Unknown ({cls})"
        ...
        disaster_type = get_disaster_name(prediction)
        ...
        # FIXED: Use the same helper function for creating disaster names
        disaster_names = [get_disaster_name(cls) for cls in disaster_classes]
        ...
        # FIXED: Ensure the predicted disaster is in the dataframe
        if disaster_type not in prob_df['Disaster Type'].values:
            # Add the predicted disaster if it's missing
            prob_df = pd.concat([
                prob_df,
                pd.DataFrame({
